...
output-[stage function](#g-stage-function) (`h_x`/`h_xu` on the continuous [tier](#g-tier),
`h_s`/`h_su` on the discrete — the two output stages every component provides)
...
(`h_x`/`h_xu` on the continuous [tier](#g-tier), `h_s`/`h_su` on the discrete)
returns a named tuple. The framework scatters each field into that port's
concretely-typed cell. Every reader — the next stage, `f`/`g`, [guards](#g-guard), wired consumers, [snapshot](#g-snapshot)
...
- An environment [component](#g-component) emits a field value (`ISAField(T_sl, p_sl,
  ...
  Inside their own [stage functions](#g-stage-function) (`h_x`/`h_xu` on the
  continuous [tier](#g-tier), `h_s`/`h_su` on the discrete) they call query
...
# discrete component — its own stage names over its own state letter
y_s      = h_s(comp, args)          # s, t, Δt [, ws]
y_su     = h_su(comp, args)         # s, u, y_s, t, Δt [, ws]
s⁺       = g(comp, args)            # s, y, u, t, Δt [, ws]
...
`f(c::LowPassFilter, (; x, u)) = …`, `h_su(c::PID, (; s, u, Δt)) = …`.
...
closed ([D-074][d-074]). `project` alone stays positional: one store in, the same store
...
The per-function name sets are **closed**, one set per function — the two
tiers' name families are disjoint by construction ([D-195][d-195]), so no set
needs a tier qualifier. Adding a name is a decision-log entry, not a
convenience. The comment in the signature block above
...
**Rule.** A stage suffix names the **dependence class**, not the argument list.
The tier's state letter alone versus that letter plus `u` is state-only versus
state-plus-input — the `y = h(x)` / `y = h(x, u)` distinction spelled in the
name, on the continuous tier as `h_x`/`h_xu` and on the discrete as
`h_s`/`h_su` ([D-195][d-195]). So "no `u` in the name" *is* the
...
The stage names therefore carry the [tier](#g-tier) with them, and so does the
update law, `f` versus `g`. A stateful leaf's declarations must agree on one
tier throughout ([§8.2][s8-2]); a leaf mixing the families — `h_x` beside `g`,
`h_s` beside `f` — is a build error, not a reading.

#### Stage roles: what each stage may see

**`h_x`/`h_s` is the no-feedthrough stage**, defined entirely by what it cannot see. Its
...
**`h_xu`/`h_su` receives all wired inputs plus the stage-1 hand-down** — its own
...
4. **Decoder-exclusive state access → stores-and-views arguments.** Step 3's
   ...
   decoder itself (today's `h_x`/`h_s`). That decoder is no longer the sole state
...
[stage functions](#g-stage-function) — `h_x`/`h_xu` on the continuous
[tier](#g-tier), `h_s`/`h_su` on the discrete, the two output stages
...
`output_types`, `events`, and the shapes of `init_x`/`init_s`/`init_m` — must be
...
**Rule.** `init_x` on the continuous [tier](#g-tier), `init_s` on the discrete,
and `init_m`, declare *by initial value*: the type is derived from the value.
...
common case: a walking producer, a frozen discrete producer and a
[root input](#g-root-input) are all admissible behind it, so substitution stays intact.
...
**Root inputs are the one place an entry types a cell.** Produced by no
component, a root input has only the consumer declaration to take a type from. The
**root-input type** is the entry evaluated at `Float64`, and only a *tight* bound
determines one. A face surfacing as a root input must therefore resolve to a
...
the root-input type is the unique concrete declaration among its consumers — two
different concrete declarations remain an error — and abstract co-consumers are
checked against it. The **root-input cells** at an activation follow the root-input type by
evaluating that same entry at the activation's `T`, which makes **seedability
schema-visible**: a `T`-entry root input is a lawful linearization `B`-matrix tap,
and a `Float64`-entry root input is *declaredly* unseedable ([§14.10][s14-10]).
...
**Why.** The direction of the meet is forced by embedding. A pinned root-input cell
...
What the mixture costs is stated where it is paid: such a root input is unseedable,
...
from `init_x` is walked: real leaves and `Real` type
parameters follow the activation scalar. `init_m` and `init_s`
pin wholesale, mirroring the discrete-producer rule. The asymmetry is
...
chicken-and-egg. Stage-1 functions (`h_x`, `h_s`) structurally receive no inputs, so
...
property is exactly what makes the derivation well-founded. Stage names do carry
the [tier](#g-tier) ([D-195][d-195]), and that is a different fact: the tag this
...
**A store needs its update.** `init_x` with no `f` method, or `init_s` with no
`g` method, is a build error: continuous state with no [flow](#g-flow), or a
discrete store nothing updates. The framework will not silently supply `ẋ = 0`, which is a
...
**[Tier](#g-tier) is declared by the whole name family.** For a **stateful**
...
declarations announce; it covers declaring both `f` and `g`, a `g` beside a
two-argument `output_types`, and the wrong-letter cases the split state letters
restore — an `h_x` or an `init_x` on a leaf whose update law is `g`, an `h_s`
or an `init_s` on one whose update law is `f`.
...
declare the [pinned](#g-walked) discrete world. Its stage names follow that
decision like any other leaf's — `h_xu` under the two-argument forms, `h_su`
under the plain ones — and [§13.7][s13-7] records why one stateless continuous
...
have to carry both classes: a field declared `E <: AbstractEngine`
...
`init_x`/`init_s`/`init_m`, `workspace`, `input_types`/`output_types`,
`events`, or any stage, `f`, `g` or `project` method.
...
`except` set and joins the input face surface, ultimately a [root input](#g-root-input) for
conditions to cover
([§14.6][s14-6]).
...
helper spelled `except = fed(sys, "aero")`, reading the assembly's own wire
...
flat state layout, [root inputs](#g-root-input). [§8][s8] states what is declared and what must hold;
...
- The `init_x`-derived state type is [walked](#g-walked) by the leaf-walk rule
  ([§8.2][s8-2]); the `init_s`- and `init_m`-derived store types pin.
...
(declared by value); the stage-1 hand-down — `y_x`, `y_s` on the discrete
[tier](#g-tier) — from the stage-1 probes' *returns* (an
...
inputs**. The build synthesizes their values via `probe_value(::Type)`:
...
root inputs are concrete by the tight-bound rule ([§8.2][s8-2]; the root-input type
...
write-side granularity rule keeps root inputs predominantly scalar, so the
...
garbage once the build finishes; probe values never double as initial root-input
...
`Simulation` must not reach its first [boundary](#g-boundary) with uninitialized root inputs;
enforcement is the pre-write `UninitializedInputs` check carried by every
...
- [root-input](#g-root-input) cells re-typed by *evaluating* the consuming
...
leaf's `s` shape; handlers against the [§5.2][s5-2] return law, key by key.
...
- **Trim** is a write-condition → [sweep](#g-sweep) → read loop on an [activation](#g-activation) — by
  ...
  *output* is framework vocabulary). `assign!` inverts from in-place mutation +
  ...
  by path, modes, [root inputs](#g-root-input) by [face](#g-face)) that the service writes and evaluates. Domain
  ...
  [value-level constructor](#g-value-level-constructor) or held directly as a rig root-input value ([§14.1][s14-1], [§14.9][s14-9]).
- **Linearization** is a `Dual` activation plus seeded sweeps: gather/scatter
  over the canonical layout replaces the hand-written
  `get_x_ss`/`assign_x_ss!` layer (the deletion discharged, [§7.1][s7-1]); root inputs are
...
- The generic service loop (vectorization, optimizer setup, bounds packing,
  ...
  root-input capture) replaces today's per-aircraft NLopt plumbing. A failed trim
...
semantically forced.** The [boundary sweep](#g-sweep)'s stage-1 block — `h_x`
and `h_s` entries alike — is order-free by definition (the
no-[feedthrough](#g-feedthrough) stage reads no `u`). The
stage-2 block gates in the [due](#g-due) discrete stages — those whose components
...
- A **[frame](#g-frame)** is a grid step `[tₙ, tₙ₊₁]`, the unit of scheduling.
  ...
  that publishes staged device writes into the root inputs, [§11.4][s11-4]), pacer
...
*exact* for a `u`-caused edge (above; [D-179][d-179]), so boundary firing is the
...
tolerance is not scale-free ([D-133][d-133]). `1e-6`, because the event time can only
...
do. Per-trial projection is rejected ([D-018][d-018]).
...
and pace-independent like any other localization outcome ([D-080][d-080]).
...
the pace-independence guarantee ([D-080][d-080]) stands untouched and the run replays
...
**Rule.** A discrete component's `h_s`/`h_su` run only at its own ticks, and
...
The split applies to **both sweep blocks**. The discrete [tier](#g-tier)'s `h_s`
entries are absent from the interior stage-1 walk exactly as its `h_su` entries
...
reads the table and writes only its own `s` store. The FCS cascade's intra-tick
...
`Δt` field of every discrete-tier bundle ([§5.2][s5-2]) — `h_s`, `h_su` and `g`
...
coefficients, a LeadLag's Tustin transform — run in `h_su`; `g` is a copy.
...
register-visible ([D-191][d-191]): an eligible-but-blocked event is the one case whose
...
are recorded in [D-154][d-154]: the per-event re-decode with a frozen round-start `u`
([D-016][d-016], [D-100][d-100], [D-152][d-152]), live-table reads under the canonical execution order, a
table copy per firing round ([D-100][d-100]), and handlers stripped of their own `y`.
...
- *Events → ticks: handled by machinery already in place.* Due discrete
  components' output stages (`h_s`/`h_su`) are gated *into* the boundary sweep
  against a due set fixed for the whole iteration ([§10.5][s10-5]). Every iteration
  round therefore refreshes them for free, against the same `s` — their `g` has
...
- *Ticks → events: structurally impossible.* A tick's output stages contribute
  ...
  current `s`. Its `g` update writes `s⁺` after the sweep, and `s⁺` is first
...
- The pure timetable script is a one-liner reading the clock out of its
  [bundle](#g-bundle) (the NamedTuple of zero-copy views a component function
  receives). That one-liner is `h_su(c, (; t)) = (; offset = profile(t))`,
...
column of [Appendix C][sC] derived from it. Where an
...
index. The second is which function is running: `h_x`/`h_s`, `h_xu`/`h_su`, `f`, `g`, a
...
h_s(::UnitDelay, (; s)) = (; … = s.v)     # publishes the stored value
...
- **Due `g` updates run.** This follows from an interval-alignment fact that
  ...
  *outgoing* transition** — at tick `t_k` it consumes the completed boundary's
  ...
  |---|---|---|---|
  | discrete | the `g` that would have produced a discrete leaf's `s(0)` never ran; the condition authored `s(0)` | `g` consumes the `t₀` samples and produces `s(1)` | the gated stages read `s(1)` |
  ...
  The outgoing work all runs, and `t₀`'s `g` has its only opportunity: `s(1)`
...
- **One new activation** ([§9.4][s9-4]): "continuous chain + `f` +
  `h_s`/`h_su` + `g`".
...
- The current update entangles outputs and next state by construction. The
  ...
  integrator state.
- Under [§5.3][s5-3] the law runs once in `h_su`, publishing paths, saturation and the
...
```julia
h_su(c::PI, (; s, u)) = (; u_cmd = u.engage ? u.u_latch : c.k_p*u.e + s.s_i)
g(c::PI, (; s, u, Δt)) = (; s_i = u.engage ? u.u_latch - c.k_p*u.e
                                           : s.s_i + c.k_i*Δt*u.e)
```
...
the continuous state and `g` only the discrete one, and `s⁺` is decoded only at
...
forbids (`g` writes only its own `s`; handlers are the sole resetters of
...
function h_su(smp::IMUSampler, (; s, u, Δt))
...
g(smp::IMUSampler, (; u)) = (Θ = u.Θ, q = u.q, Υ = u.Υ, V = u.V)   # the latch
```
...
[boundary zero](#g-boundary-zero) discharges the rest: its [due](#g-due) `g` latches `s ← integrals(t₀)` for
...
timeline closes consistently: the sampler's `h_su` decodes `s` (the `t_{k-1}` latch)
...
values from its *[feedthrough](#g-feedthrough)* stage (`h_su` reads `u`, so the latch [port](#g-port) carries
the current tick's values, ZOH until the next; an `h_s`-published latch would be
...
sampler `h_su`; sampler `h_su` → the integrals' `f`-edge, [§5.4][s5-4]). The "reset"
...
- **Same-tick reset consumption** ([§15.2][s15-2]) — *discrete tier*. A commanded reset
  ...
  and `g` stores the matching `s⁺`. A reset honored only in `g` reaches the
...
Three columns place each kind. **Severity** is a property of the kind, read
as `severity(d)` ([§13.2][s13-2]), and takes one of two values:
...
| `DeclarationOnWrongTier` | component path, the offending declaration (`f`/`g`, a state or stage name from the wrong family — `init_x`/`h_x`/`h_xu` against `init_s`/`h_s`/`h_su`, [D-195][d-195] — `events`, `init_m`, `project`, or a `workspace`/`output_types` arity), the tier the leaf's other declarations announce | [§5.2][s5-2], [§8.2][s8-2], [§8.5][s8-5] | error | build | collected |
...
| `UserCodeFraming` | component path, which function, the probe context including synthesized inputs; the original exception as `cause` | [§13.2][s13-2] | error | build | fail-fast |
...
| `TapResolution` | tap set (`x`/`u`/`y`), selector kind, path, field, optional index, candidates; for a declaredly-unseedable root input, the pinning consumer's path and its `input_types` entry | [§14.10][s14-10] | error | service | collected |
| `TrimProblemInvalid` | the offending `TrimProblem` field, the names or types in hand (a key-set or field-type mismatch; never a field-order difference) | [§14.7][s14-7], [§14.8][s14-8] | error | service | collected |
| `TrimCommitEvents` | the events fired at boundary zero: component paths and event names; the same list rides the `TrimReport` | [§14.8][s14-8] | warning | service | logged |
...
receive: `h_x`/`h_xu`/`h_s`/`h_su`/`f`/`g`/guard/handler/`project` (one closed
set per name, the tiers' families being disjoint, [§5.2][s5-2]), with the
...
output stages (suffix = dependence class, not argument list), `x` the continuous
...
<a id="g-periodic-discrete-component"></a>**periodic discrete component** — a leaf with state `s`, update `g`
...
output stages, `h_x`/`h_s` (no `u` in the bundle, hence structurally no
feedthrough) and `h_xu`/`h_su`; feedthrough is thereby declared by signature,
...
[d-001]: framework_decisions.md#d-001--hybrid-causal-formalism-with-two-tier-events-and-projection
[d-004]: framework_decisions.md#d-004--immutable-value-signals-in-a-typed-signal-table
[d-005]: framework_decisions.md#d-005--reject-algebraic-loops-at-build
[d-006]: framework_decisions.md#d-006--two-stage-structural-feedthrough-via-h_xh_xu
[d-007]: framework_decisions.md#d-007--aggregation-mechanism
[d-008]: framework_decisions.md#d-008--function-valued-environment-signals-with-the-handle-pattern
[d-011]: framework_decisions.md#d-011--eltype-genericity-on-the-continuous-path
[d-012]: framework_decisions.md#d-012--set-propagation-tracers-and-scc-based-cycle-diagnostics
[d-013]: framework_decisions.md#d-013--immutable-discrete-state-via-cells-workspace-and-snapshot
[d-015]: framework_decisions.md#d-015--fused-evaluation-of-derivatives-and-outputs
[d-016]: framework_decisions.md#d-016--uniform-component-interfaces-via-h_x-and-per-event-re-decode
[d-017]: framework_decisions.md#d-017--framework-owned-simulation-loop-with-a-stepper-seam
[d-018]: framework_decisions.md#d-018--tier-2-event-localization-via-dense-output-and-bracketed-root-finding
[d-019]: framework_decisions.md#d-019--harmonic-tick-grid-with-virtual-assemblies-and-rate-scopes
[d-020]: framework_decisions.md#d-020--boundary-event-phase-iterates-to-quiescence
[d-021]: framework_decisions.md#d-021--pacer-piecewise-affine-wall-clock-mapping-with-bounded-debt
[d-022]: framework_decisions.md#d-022--periphery-architecture-no-shared-mutable-model
[d-023]: framework_decisions.md#d-023--snapshot-publication-via-atomic-acquire-release
[d-024]: framework_decisions.md#d-024--inbound-staging-per-device-atomic-batch-cells
[d-025]: framework_decisions.md#d-025--one-uniform-device-kind
[d-026]: framework_decisions.md#d-026--gui-write-path-panels-name-their-own-ports
[d-027]: framework_decisions.md#d-027--pacer-coarse-phase-uses-task-yielding-sleep
[d-028]: framework_decisions.md#d-028--next-snapshot-wait-via-monotonic-counter-and-condition-variable
[d-029]: framework_decisions.md#d-029--input-trace-on-by-default
[d-031]: framework_decisions.md#d-031--mid-run-mutation-doctrine-staging-and-control-commands-only
[d-032]: framework_decisions.md#d-032--component-declaration-trait-layer-with-probe-checked-schema-authority
[d-033]: framework_decisions.md#d-033--declaration-inventory-by-value-by-type-by-allocation-registers
[d-034]: framework_decisions.md#d-034--contract-visibility-declared-fields-are-public
[d-035]: framework_decisions.md#d-035--stores-and-views-components-read-zero-copy-view-bundles
[d-036]: framework_decisions.md#d-036--table-mechanics-stage-returns-are-namedtuples-of-port-values
[d-037]: framework_decisions.md#d-037--aggregation-by-explicit-summing-junctions
[d-038]: framework_decisions.md#d-038--snapshot-and-log-derived-trajectory-primary-trace-header
[d-039]: framework_decisions.md#d-039--assembly-declaration-is-type-based
[d-040]: framework_decisions.md#d-040--slash-string-paths-as-the-canonical-path-form
[d-041]: framework_decisions.md#d-041--dedicated-exports-for-assembly-faces
[d-042]: framework_decisions.md#d-042--rates-declaration-on-immediate-children-only
[d-043]: framework_decisions.md#d-043--computed-exports-via-ordinary-code-and-faces
[d-044]: framework_decisions.md#d-044--slot-exclusivity-and-the-write-surface-rule
[d-045]: framework_decisions.md#d-045--periphery-input-semantics-derived-liveness-conditioning-mappings-edge-logic
[d-046]: framework_decisions.md#d-046--face-names-are-opaque-string-tokens-slash-is-reserved-for-structure
[d-047]: framework_decisions.md#d-047--stage-widgets-on-interaction-events-not-every-pass
[d-049]: framework_decisions.md#d-049--standalone-buildworld-artifact-wrapped-by-simulation
[d-051]: framework_decisions.md#d-051--synthesize-probe-inputs-via-a-probe_valuetype-fallback-chain
[d-052]: framework_decisions.md#d-052--scope-dual-probing-to-each-activations-executable-set
[d-053]: framework_decisions.md#d-053--bake-one-always-on-conformance-check-at-every-table-write
[d-054]: framework_decisions.md#d-054--producers-determine-activation-types-consumers-stay-generic
[d-055]: framework_decisions.md#d-055--require-strict-local_types-declaration-for-cross-stage-cells
[d-056]: framework_decisions.md#d-056--uphold-the-two-kind-taxonomy-against-the-integrate-and-dump-challenge
[d-057]: framework_decisions.md#d-057--batch-declarative-check-violations-abort-on-first-user-code-exception
[d-058]: framework_decisions.md#d-058--diagnostics-as-structured-values-under-one-builderror-carrier
[d-059]: framework_decisions.md#d-059--catch-runtime-failures-at-one-boundary-site-into-steperror
[d-060]: framework_decisions.md#d-060--graceful-termination-is-model-state-never-an-exception
[d-063]: framework_decisions.md#d-063--conditions-are-path-addressed-sparse-overlays-on-init_-defaults
[d-064]: framework_decisions.md#d-064--compose-per-component-init-by-pull-via-fragment-functions
[d-065]: framework_decisions.md#d-065--fragments-form-a-lazy-inert-tree-resolved-against-the-build
[d-067]: framework_decisions.md#d-067--boundary-zero-runs-the-macro-sequence-with-an-empty-integrate
[d-068]: framework_decisions.md#d-068--enforce-slot-totality-at-the-initcommit-service-boundary
[d-069]: framework_decisions.md#d-069--trim-problem-spelling-namedtuples-residual-vector-exact-ad-jacobians
[d-070]: framework_decisions.md#d-070--trim-service-in-house-dense-lm-behind-a-swappable-backend
[d-073]: framework_decisions.md#d-073--companion-sketches-carry-the-settled-condition-algebra-design
[d-074]: framework_decisions.md#d-074--hand-off-component-function-arguments-as-one-named-bundle
[d-075]: framework_decisions.md#d-075--name-flowupdateoutput-stages-by-letter-and-dependence-class
[d-077]: framework_decisions.md#d-077--allocate-workspace-via-a-per-activation-workspace-method
[d-079]: framework_decisions.md#d-079--type-declarations-concretely-resolved-by-an-activation-leaf-walk
[d-080]: framework_decisions.md#d-080--keep-tier-2-event-detection-pace-independent
[d-083]: framework_decisions.md#d-083--bind-output-device-reads-to-snapshot-paths-not-just-faces
[d-084]: framework_decisions.md#d-084--drop-the-unconnected-output-warning
[d-086]: framework_decisions.md#d-086--compile-the-executors-schedule-into-unrolled-statically-typed-entries
[d-089]: framework_decisions.md#d-089--route-supervisor-gains-and-resets-through-ordinary-ports
[d-090]: framework_decisions.md#d-090--return-handler-updates-as-bundle-law-namedtuples
[d-091]: framework_decisions.md#d-091--override-t_endstop_on-per-run-at-run
[d-093]: framework_decisions.md#d-093--spawn-device-tasks-per-run-not-per-attach
[d-094]: framework_decisions.md#d-094--close-the-state-leaf-vocabulary-to-plain-scalars-and-sarrays
[d-100]: framework_decisions.md#d-100--freeze-u-at-round-start-for-within-round-event-visibility
[d-101]: framework_decisions.md#d-101--implement-replay-as-the-ordinary-loop-with-two-substitutions
...
[d-141]: framework_decisions.md#d-141--continuous-state-resets-are-events-owned-by-the-reimplemented-pivector
[d-142]: framework_decisions.md#d-142--stage-code-must-be-total-over-type-valid-inputs
[d-144]: framework_decisions.md#d-144--rename-the-computed-exports-helper-faces-to-passthrough
[d-145]: framework_decisions.md#d-145--deduplicate-pass-through-except-lists-with-a-shared-feed-list-idiom
[d-146]: framework_decisions.md#d-146--rename-facesselectors-to-claimsreads-on-the-binding-interface
[d-147]: framework_decisions.md#d-147--split-the-sweep-into-static-interior-and-boundary-variants
[d-150]: framework_decisions.md#d-150--make-the-service-the-sole-authority-on-convergence
...
[d-157]: framework_decisions.md#d-157--check-for-nonfinite-x-immediately-after-integrate
...
[d-169]: framework_decisions.md#d-169--y_xy_z-carry-stage-1-ports-only-auto-published-excluded
[d-170]: framework_decisions.md#d-170--split-assembly-connections-into-childinputoutput-declarations
[d-171]: framework_decisions.md#d-171--rename-passthrough-to-input_passthrough
[d-173]: framework_decisions.md#d-173--fuse-the-discrete-state-letter-z-into-x
...
[d-183]: framework_decisions.md#d-183--retire-workspace-poisoning
...
[d-186]: framework_decisions.md#d-186--legalize-absolute-declarations-in-any-scope-via-anchors
[d-187]: framework_decisions.md#d-187--make-the-bound-schedule-a-named-artifact-with-exact-grid-diagnostics
[d-190]: framework_decisions.md#d-190--reject-a-separate-derivative_type-declaration
