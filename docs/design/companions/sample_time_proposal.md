...
(`extensions.md`), which sketched the same territory before the pipeline was
...
- **Boundary zero**: `idx = 0` makes the gate `(0 − Φ) % D == 0`, which by the
  ...
  cells hold the values the build probe populated from `s_init` ([§9.3][s9-3]) — a coherent
...
extensions charter's triggered-updates item (`extensions.md` section 1.2,
...
- **t = 0**: due = {ctrl} (the boundary-zero rule of section 5.5). The sensors have
  never ticked; ctrl reads their build-probed `s_init` cells for the first
  millisecond.
...
- **Multiples of 20 ms**: due = {ctrl}, whose `y_direct` reads sensor cells written at
