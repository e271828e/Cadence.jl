...
`spec.md` [§5.3][s5-3] (step-boundary semantics), [§10.6][s10-6] (event iteration)
...
- **Per-event re-decode with a frozen round-start `u`** (rejected; the prior
  design of record, [D-016][d-016]/[D-100][d-100]/[D-152][d-152]): two freshness classes inside one
