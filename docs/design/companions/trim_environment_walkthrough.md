...
trim service; it is recorded as a `TrimReport`-level nicety in `pending.md`, because the natural place for it is the same report that
...
`pending.md` as a `TrimReport`-level addition.
