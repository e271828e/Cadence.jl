...
`spec.md` [§11.3][s11-3] (root inputs, claims, the roster freeze), [§11.4][s11-4] (staging and the drain), [§11.7][s11-7] (GUI write path),
...
harness writer as the sole derived surface, uniform sparse trace records,
...
the harness remainder ([section 5](#5-two-modes-of-use-not-more-machinery)), and that partition is a static,
...
(the harness cell last, [section 5](#5-two-modes-of-use-not-more-machinery)):
...
## 5. Two modes of use, not more machinery

The one mechanism above is used in two sanctioned ways, and neither adds
machinery:

- **The claimed mode**: enumerate faces in your binding — or declare
...
acquired; the harness writer's is the derived remainder — and under the
...
- `ClaimedFaceEntry` — a harness-writer violation, and only that:
...
[d-096]: ../decisions.md#d-096--define-the-harness-writer-stagelateststep-duration
[d-106]: ../decisions.md#d-106--freeze-the-device-roster-for-the-duration-of-a-run
[d-107]: ../decisions.md#d-107--match-trace-record-density-to-batch-density-not-surface-width
[d-174]: ../decisions.md#d-174--re-class-the-gui-as-an-ordinary-enumerated-writer
