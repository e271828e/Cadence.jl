...
gate reading `u`/`m` — is blessed by [D-179][d-179], so `u`-reading localized guards are a
...
budgeted re-firing ([D-181][d-181]) without changing a single step.
...
[d-182]: ../decisions.md#d-182--add-a-θ0-validation-probe-to-the-localization-trigger
