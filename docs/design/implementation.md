...
| `src/executor.jl` | entries (each carrying its component's path, for the write's diagnostic, an event entry its event name beside it (D-249)), the chunked unrolled walk, the interior/boundary split, the `(idx − Φ) % D` gate and boundary zero's `ESTABLISH` beside it, the event set with its registers and the guard/fire/project walks over the executor's buffers the caller hands them (D-261), and the execution cursor every entry stores into, carrying the loop's stop hit beside the frame it names (§13.5, D-255) | §5.3, §9.5, §9.7, §10.4–§10.6, §13.4, §13.5, §14.5, D-059, D-205, D-235, D-249, D-255, D-261 |
