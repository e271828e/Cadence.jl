...
| `test/fixtures.jl` | the suite's fixtures: the coverage component set, the named assemblies, the devices and bindings, the `condition` fragment-function idiom, `Pendulum`, the `ForgottenImport` module, the forgotten-import fixtures, importing nothing — user material, and no name here is known to `src/` | — |
