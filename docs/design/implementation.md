...
| `src/leaves.jl` | the leaf walk: flatten / reconstruct / the activation retype, and `leaf_names`' dotted spelling of a flat position, embed-accept's relation `_accepts` (D-166) and the checked state write `flatten_state!` (D-235) | §7.1, §7.2, §9.5, §13.4, D-166, D-235 |
