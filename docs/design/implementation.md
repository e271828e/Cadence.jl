...
| `src/sim.jl` | `Run{T}` — §12.6's run state, `t₀`, the mode, the log and the trace fixed by the door that built it, the policy and the termination record written into it, with `closed(run)` beside it (D-255) — and the mutable `Simulation` holding the deployment, the executor, the run, the plane and the control, plus the nine fields stage 3 of increment 47 moves to their owners (D-256, §12.1), the materialization `Simulation(deployment, T)` with its own keywords under `ArgumentInvalid`, its placeholder run carrying the recording flags to `init!`, and the two sugar forms defined as the composition (D-254), `warnings(::Simulation)` as the concatenation (D-250), the boundary macro-sequence and the §10.6 event phase with its `FiringBudget` degradation, `init!`, `run!`/`step!`, `replay!` over the one shared run body — the three doors that build a run, `_open_trajectory!` and `_open_run!` shared by the two that open a trajectory —, `attach!`/`detach!`, staging/drain/publication with the drain's replay substitution, §12.6's input mode (`mode(sim)`, `to_time`, `live!`, the mode being one of the run's `const` fields so a change of mode is a change of run), the `StopPolicy` each advance builds and validates per call and binds on the run — `t_end` and `stop_on` are keywords of `run!`/`replay!`/`step!`, never of the constructor, and the unbounded run's advisory is raised at `run!` (D-255) —, the lifecycle and termination record, the frame loop's one catch site with the species rule — the runtime bundle-field match in it (§13.2, §13.4, D-248), its stage-1 names off the `Dataflow` — and the interrupt carve-out and its second host around boundary zero, the seam's `isfinite` sweep over `x` as the boundary's first act, the accessors | §10.2–§10.6, §11.1–§11.4, §11.8, §12.1–§12.7, §13.2, §13.4–§13.6, §14.5, §14.6, D-059, D-101, D-157, D-203, D-218, D-219, D-221, D-223, D-232, D-233, D-248, D-253, D-255 |
...
| `src/dataplane.jl` | the compiled writer and staging cells, the drain, snapshots and the log with re-decimation, the typed diagnostic kinds and cells — `UnboundedRun` among them, the loop's own advisory (§13.5, D-255) — and the published `FrameworkStatus` every snapshot carries | §11.1–§11.4, §11.8, §12.4, §12.6, §13.2, §13.5, D-023, D-038, D-137, D-255 |
| `src/trace.jl` | the input trace: `TraceHeader` captured at `init!` (resolved stores, root inputs, the run's `Deployment` and its `t₀`) and the mutable `Trace{T}` behind it — the header plus two lists that grow in place, the writers' schemas and one sparse record per drained batch, and the length a replay reads its bound off (D-255) —, `TraceRegister` reduced to the drain's bookkeeping and held by the plane, `_install_writers!`'s growth rule, `trace(sim)` handing back a detached value, and replay's up-front entry pass (`_compile_feed`) validating header, schemas and records, both stages collecting since D-217, into the `ReplayFeed` the drain reads — the header's deployment half being one `==` with `_walk_deployment!` naming what it refused, the schedule's rows and scopes by path and column (§12.7) | §11.5, §12.6, §12.7, §14.5, D-029, D-038, D-101, D-176, D-217, D-218, D-254, D-255 |
| `src/roster.jl` | device/binding traits and conformance, the roster, both claim sources, the harness writer, and §11.5's trace register on the plane beside them — the drain's bookkeeping, held where the writers are, and read by `reclaim!` rather than passed to it (D-255) | §11.3, §11.4, §11.5, §11.6, D-255 |
...
| `src/devices.jl` | the device contract, the handle, the task wrapper, the init bracket and the tail under `join_timeout`; the two lifecycle gates, the readers' and the roster's; §13.5's `StopPolicy`, defined here for include order alone, and the termination record carrying the terminating advance's policy beside its source — the record living on the `Run` and no longer on `Control`, which keeps the stop word, the lifecycle and the wait (D-255) | §11.1, §11.3, §11.6, §12.1–§12.4, §13.5, §13.6, D-198, D-232, D-233, D-244, D-255 |
