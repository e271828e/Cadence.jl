...
| `src/roster.jl` | device/binding traits and conformance, the roster, both claim sources, the harness writer, and §11.5's trace register on the plane beside them — the drain's bookkeeping, held where the writers are, and read by `reclaim!` rather than passed to it (D-255); the loop is a writer too, so the plane holds its diagnostic cell and account, and §11.2's published holder with them (D-256) | §11.2, §11.3, §11.4, §11.5, §11.6, §11.8, D-255, D-256 |
...
| `src/devices.jl` | the device contract, the handle, the task wrapper, the init bracket and the tail under `join_timeout`, which `Control` carries (D-256); the two lifecycle gates, the readers' and the roster's; §13.5's `StopPolicy`, defined here for include order alone, and the termination record carrying the terminating advance's policy beside its source — the record living on the `Run` and no longer on `Control`, which keeps the stop word, the lifecycle, the wait and the shutdown cap (D-255, D-256) | §11.1, §11.3, §11.6, §12.1–§12.4, §13.5, §13.6, D-198, D-232, D-233, D-244, D-255, D-256 |
